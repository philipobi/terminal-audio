...
UI::UI(int nbars, int bar_height, int bar_width, int bar_margin, int y, int x)
    : nbars(nbars), bar_height(bar_height), bar_width(bar_width),
      bar_margin(bar_margin), height(bar_height + 2 * bar_margin + 1),
      width(nbars * bar_width + (nbars + 1) * bar_margin), y(y), x(x),
      amplitudes(new double[nbars]{0}) {
  int i, j;

  // create bar segments
  partition_fractions<int>(nsegments, segment_ratios, bar_height,
                           segment_heights);

  barSegments.reserve(3);
  int y_off = bar_margin;
  for (i = 0; i < nsegments; i++) {
    barSegments.emplace_back(segment_heights[i], width - 2 * bar_margin,
                             y + y_off, x + bar_margin);
  ...
  // create footers
  Window **pWindows[4] = {&pFooter, &pLabels, &pRaw, &pNorm};
  for (i = 0; i < 4; i++) {
    *pWindows[i] =
        new Window(1, width - 2 * bar_margin, y + y_off, x + bar_margin);
    y_off++;
  }
  ...
  // create progress bar
  pProgressBar = new Window(3, player_width + 2, y, x + width + 2);
}
UI::~UI() {
  for (auto &barSegment : barSegments)
...
void UI::update_amplitudes(const double *amplitudes_raw) {
  ...
  if (PLAYER_DEBUG) {
    for (i = 0, pAmp = amplitudes, pAmp_raw = amplitudes_raw; i < N_BINS;
         i++, pAmp++, pAmp_raw++) {
      mvwprintw(pNorm->p_win, 0, i * (bar_margin + bar_width), "%.2f", *pAmp);
      mvwprintw(pRaw->p_win, 0, i * (bar_margin + bar_width), "%.2f",
                *pAmp_raw);
    }
    wrefresh(pRaw->p_win);
...
void UI::update_player(const PlaybackInfo *pPlaybackInfo) {
  ...
  double width = round(player_width * pPlaybackInfo->audioFrameCursor /
                       double(pPlaybackInfo->audioFrameSize));
  mvwprintw(pRaw->p_win, 0, 0, "%f", width);
  pProgressBar->clear();
  pProgressBar->stack_n_horizontal(1, width, '#', 1);
